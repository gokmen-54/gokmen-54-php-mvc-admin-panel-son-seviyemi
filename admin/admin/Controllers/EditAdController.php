<?php 
if(empty($_GET["token"])) { echo '<script type="text/javascript">window.history.go(-1)</script>'; } else { $token_ads = $_GET["token"]; }
...
if(isset($_POST["update"])){
    ...
    if(empty($company) || empty($enddate) || empty($description)){
        ...
        $query_ads = mysqli_query($db,"SELECT * FROM shop_storyads WHERE token = '$token_ads'"); $data_ads = mysqli_fetch_array($query_ads);
        ...
        else { $random_image = substr(str_shuffle("a0b1c2d3e4f5g6h7i8j9"), 0, 19); $image = basename($_FILES['image']['name']); move_uploaded_file($_FILES['image']['tmp_name'],  "../public/images/ads/" . $random_image . $image); $rule_image = file_get_contents($_FILES['image']['tmp_name']); $query_image = getimagesize($_FILES['image']['tmp_name']); $update_image = mysqli_query($db,"UPDATE shop_storyads SET image = '$random_image$image' WHERE token = '$token_ads'"); }

        $update_ads = mysqli_query($db,"UPDATE shop_storyads SET company = '$company', enddate = '$enddate', description = '$description' WHERE token = '$token_ads'");
