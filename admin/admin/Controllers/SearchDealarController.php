<?php
    ...
    if(isset($_POST["search"]))
    {
        ...
        {
            $query_search = mysqli_query($db,"SELECT * FROM shop_users WHERE fullname LIKE '%$fullname%' ORDER BY token DESC");
            ...
            if(mysqli_num_rows($query_search) > 0)
            {
                while($data_search = mysqli_fetch_array($query_search))
                {
                    ...
                    <tr>
                    <td>
                    <div class="main__table-text">'.$data_search['fullname'].'</div>
                    </td>
                    <td>
